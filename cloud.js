...
var SnapCloud = new Cloud(
  location.origin === "https://physics.c2stem.org" ||
  location.origin === "https://physdev.c2stem.org"
    ? "https://cloud.c2stem.org/SnapCloud/"
    : location.origin + "/SnapCloud/"
);
      ...
      if (request.readyState === 4) {
        if (request.responseText) {
          if (request.responseText.indexOf("ERROR") === 0) {
            errorCall.call(this, request.responseText, "Signup");
          } else {
            callBack.call(null, request.responseText, "Signup");
          }
        } else {
          errorCall.call(
            null,
...
Cloud.prototype.getPublicProject = function (id, callBack, errorCall) {
  ...
  try {
    request.open(
      "GET",
      (this.hasProtocol() ? "" : "http://") + this.url + "RawPublic" + "?" + id,
      true
    );
    request.setRequestHeader(
      "Content-Type",
    ...
    request.onreadystatechange = function () {
      if (request.readyState === 4) {
        if (request.responseText) {
          if (request.responseText.indexOf("ERROR") === 0) {
            errorCall.call(this, request.responseText);
          } else {
  ...
  try {
    ...
    request.onreadystatechange = function () {
      if (request.readyState === 4) {
        if (request.responseText) {
          if (request.responseText.indexOf("ERROR") === 0) {
            errorCall.call(this, request.responseText, "Reset Password");
          } else {
            callBack.call(null, request.responseText, "Reset Password");
          }
        } else {
          errorCall.call(
            null,
    ...
    request.onreadystatechange = function () {
      if (request.readyState === 4) {
        if (request.responseText) {
          ...
          myself.session = request
            .getResponseHeader("MioCracker")
            .split(";")[0];
          // set the cookie identifier:
          myself.limo = this.getResponseHeader("miocracker").substring(
            9,
          ...
          if (myself.api.logout) {
            ...
            errorCall.call(null, request.responseText, "connection failed");
          }
        } else {
          errorCall.call(null, myself.url, localize("could not connect to:"));
        }
      }
    };
    request.send(usr);
...
Cloud.prototype.saveProject = function (ide, callBack, errorCall) {
  ...
  media = ide.hasChangedMedia ? ide.serializer.mediaXML(ide.projectName) : null;
  ...
  myself.reconnect(function () {
    myself.callService(
      ...
      errorCall,
      [ide.projectName, pdata, media, pdata.length, media ? media.length : 0]
    );
  }, errorCall);
};
...
Cloud.prototype.logout = function (callBack, errorCall) {
  this.callService("logout", callBack, errorCall);
};
    ...
    request.onreadystatechange = function () {
      if (request.readyState === 4) {
        if (request.responseText) {
          ...
          errorCall.call(null, url, "no response from:");
        }
      }
    };
    request.send(null);
...
Cloud.prototype.callService = function (
  ...
  if (!service) {
    errorCall.call(null, "service " + serviceName + " is not available", "API");
    return;
  }
