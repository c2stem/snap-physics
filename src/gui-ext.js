...
IDE_Morph.prototype._rawOpenBlocksString =
...
IDE_Morph.prototype.rawOpenBlocksString = function (str, name, silently) {
  ...
  if (silently) {
    ...
    this.refreshPalette();
    this.showMessage(
      "Imported Blocks / Message Types Module" +
        (name ? ": " + name : "") +
        ".",
      2
    );
...
IDE_Morph.prototype.isMobileDevice = function () {
  var smallSideMax = 420,
    bigSideMax = 800;
  var hasSmallPortraitDims =
    screen.width <= smallSideMax && screen.height <= bigSideMax;
  var hasSmallLandscapeDims =
    screen.width <= bigSideMax && screen.height <= smallSideMax;

  return (
    (hasSmallPortraitDims || hasSmallLandscapeDims) &&
...
IDE_Morph.prototype.mobileMode.computeControlPos = function (targetRect) {
  ...
  if (this._stackMode === "v") {
    ...
    controls.origin.x =
      targetRect.bottomLeft.x + (targetRect.width - controls.width) / 2;
...
IDE_Morph.prototype.requestProjectReload = async function (reason) {
  ...
  const confirmed = await this.confirm(
    message,
    localize("Project Reload Required")
  );
  if (confirmed) {
...
LibraryDialogSource.prototype = Object.create(
  SaveOpenDialogMorphSource.prototype
);
...
OfficialLibrarySource.prototype.list = function () {
  ...
  this.ide.getURL(this.ide.resourceURL("libraries", "LIBRARIES"), (txt) => {
...
CloudLibrarySource.prototype.save = async function (item) {
  ...
  request.open(
    "POST",
    `${SERVER_URL}/api/v2/libraries/user/${username}/${name}`,
    true
  );
...
LibraryDialogMorph.prototype.init = function (ide, name, xml, notes) {
  ...
  LibraryDialogMorph.uber.init.call(this, task, "Library", sources, null, {
    name,
    notes,
  });
...
LibraryDialogMorph.prototype.buildContents = function () {
  ...
  if (this.task === "open") {
    const openButton = this.buttons.children.find(
      (btn) => btn.action === "openItem"
    );
...
LibraryDialogMorph.prototype.setPreview = async function (item) {
  ...
  if (this.hasCached(cacheKey)) {
    ...
    this.cacheLibrary(cacheKey, this.ide.serializer.loadBlocks(libraryXML));
    this.displayBlocks(cacheKey);
  }
...
LibraryDialogMorph.prototype.fixLayout = function () {
  ...
  if (this.body) {
    ...
    this.notesField.setExtent(
      new Point(this.body.right() - this.listField.right() - thin, 100)
    );
