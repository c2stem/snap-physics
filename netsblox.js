...
NetsBloxMorph.prototype.cloudMenu = function () {
  ...
  if (SnapCloud.username && this.room.isOwner()) {
    menu.addLine();
    menu.addItem("Collaborators...", "manageCollaborators");
  }
...
NetsBloxMorph.prototype.newProject = function (projectName) {
  ...
  var myself = this,
    callback = function (info) {
      ...
      return SnapActions.openProject().then(function () {
...
NetsBloxMorph.prototype.projectMenu = function () {
  ...
  if (shiftClicked) {
    ...
    menu.items.splice(10, 0, [
      ...
      function () {
        ...
        var rpcErrWatcher = detect(myself.stage.children, function (child) {
          return (
            child instanceof WatcherMorph && child.getter === "reportRPCError"
          );
        });
  ...
  if (
    this.stage.deletableMessageNames().length &&
  ...
  item = [
    ...
    function () {
      var names = this.getMediaList("rpc"),
        ...
        mediaMenu = new MenuMorph(myself, headerText);
      ...
      mediaMenu.popup(
        myself.world(),
        myself.controlBar.projectButton.bottomLeft()
      );
    },
...
NetsBloxMorph.prototype.requestAndroidApp = function (name) {
  ...
  projectXml = encodeURIComponent(this.serializer.serialize(this.stage));
...
NetsBloxMorph.prototype.openRoomString = function (str) {
  ...
  return SnapCloud.importProject(name, role, roles).then(function (state) {
    ...
    return SnapActions.openProject(projectXml).then(function () {
      msg.destroy();
...
NetsBloxMorph.prototype.openCloudDataString = function (model, parsed) {
  ...
  return IDE_Morph.prototype.openCloudDataString
    .call(this, str)
    .then(function () {
      myself.sockets.updateRoomInfo();
    });
...
NetsBloxMorph.prototype.cloudSaveError = function () {
  ...
  return function (response, url) {
    ...
    var explanation =
      "Unable to save. Please export project if the problem persists.";
...
NetsBloxMorph.prototype.saveProjectToCloud = function (name) {
  ...
  overwriteExisting = function (overwrite) {
    var contentName = myself.room.hasMultipleRoles()
      ? myself.room.getCurrentRoleName()
      : myself.room.name;
    if (name) {
...
NetsBloxMorph.prototype.droppedText = function (aString, name) {
  if (aString.indexOf("<rpc") === 0) {
    ...
    return SnapActions.openProject(aString).then(function (project) {
...
NetsBloxMorph.prototype.rawLoadCloudProject = function (project, isPublic) {
  ...
  var msg = this.showMessage("Opening project...");
  return SnapActions.openProject(project.SourceCode).then(function () {
...
NetsBloxMorph.prototype.snapMenu = function () {
  ...
  menu.addLine();
  menu.addItem("NetsBlox website", function () {
    window.open("https://netsblox.org", "NetsBloxWebsite");
  });
  menu.addItem("Snap! manual", function () {
    var url = myself.resourceURL("help", "SnapManual.pdf");
    window.open(url, "SnapReferenceManual");
  });
  menu.addItem("Source code", function () {
    window.open("https://github.com/netsblox/netsblox");
  });
  menu.addLine();
  menu.addItem("Report a bug", "reportBug");
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4 && !silent) {
      if (request.status > 199 && request.status < 400) {
        ...
        myself.cloudError()(
          url,
          localize("bug could not be reported:") + request.responseText
        );
      }
    }
  };
...
NetsBloxMorph.prototype.showBugReportScreenshot = function (report) {
  ...
  pic.onload = function () {
    var maxSize = myself.extent().divideBy(1.5),
    ...
    new DialogBoxMorph().inform("Screenshot", null, myself.world(), screenshot);
  };
  ...
  inp.addEventListener(
    ...
    function () {
      ...
      reader.onloadend = function (result) {
        ...
        dialog.ask(localize("Bug Report"), msg, myself.world(), choices);
...
NetsBloxMorph.prototype.promptCollabInvite = function (params) {
  // id, room, roomName, role
  ...
  dialog.askYesNo("Collaboration Invitation", localize(msg), this.world());
};
...
NetsBloxMorph.prototype.showUpdateNotification = function () {
  var msgText = localize(
    "Newer Version of NetsBlox Available: Please Save and Refresh"
  );
